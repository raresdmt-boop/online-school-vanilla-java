...
public class StudentView {
    ...
    void cartiStudent() {
        verificaLogat();
        List<Book> books = booksService.cartileStudentului(logat.getID());
        for (int j = 0; j < books.size(); j++) {
            System.out.println(books.get(j).getBookName());
        }
    }
    void register() {
        System.out.println("Introduceti prenumele: ");
        studentNou.setFirstName(sc.nextLine());
        System.out.println("Introduceti numele de familie: ");
        studentNou.setLastName(sc.nextLine());
        System.out.println("Introduceti email: ");
        studentNou.setEmail(sc.nextLine());
        System.out.println("Introduceti password: ");
        studentNou.setPassword(sc.nextLine());
        System.out.println("Introduceti varsta: ");
        studentNou.setAge(Integer.parseInt(sc.nextLine()));

        studentService.addStudent(studentNou);
        System.out.println("Studentul " + studentNou.getFirstName() + " " + studentNou.getLastName() + " a fost inregistrat");
    ...
    boolean verificaApartenenta(Book book) {
        return logat.getID() == book.getStudentID();
    }
    void stergeCarte(){
        verificaLogat();
        System.out.println("Care carte doriti sa stergeti?");
        Book book = booksService.getBookByName(sc.nextLine());
        if(book.getStudentID()==logat.getID()) {
            booksService.stergeCarte(book.getId());
            System.out.println("Cartea "+book.getBookName()+" a fost stearsa");
        }else{
            System.out.println("Nu este cartea dvs.");
        }
    }
    void afiseazaCursuri() {
        verificaLogat();
        ArrayList<Enrollment> enrollStudent = enrollmentService.getEnrollments(logat.getID());
        if(!enrollStudent.isEmpty()) {
            for (int i = 0; i < enrollStudent.size(); i++) {
                int courseID = enrollStudent.get(i).getCourseID();
    ...
    void inscriereLaCurs() {
        ...
        if (course == null) {
            ...
            enrollmentService.createEnrollment(logat.getID(), course.getId());
            System.out.println("Studentul "+logat.getFirstName());
            System.out.println("V-ati inscris cu succes la cursul " + course.getName());
        }
    }
    void creeazaCurs() {
        verificaLogat();
